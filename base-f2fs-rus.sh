...
do
 ...
 if [[ $x_pacstrap == 1 ]]; then
  clear
 pacstrap /mnt base linux linux-headers f2fs-tools dhcpcd  which inetutils netctl base-devel wget  efibootmgr nano  linux-firmware wpa_supplicant dialog
...
do
 ...
 if [[ $x_pacstrap == 1 ]]; then
  clear
  pacstrap /mnt base linux linux-headers f2fs-tools dhcpcd which netctl inetutils  base-devel  wget linux-firmware  nano wpa_supplicant dialog
